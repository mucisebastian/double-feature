import { startOfDay, addDays, differenceInDays, addHours, parseISO, isAfter } from 'date-fns';
...
/**
 * Checks if a date should be included in the archive
 * (i.e., it's on or after the archive start date)
 */
export function isArchiveDate(date: Date): boolean {
  const archiveStartDate = parseISO('2025-02-20');
  return isAfter(date, archiveStartDate) || date.toDateString() === archiveStartDate.toDateString();
}
