...
export default function ArchivePage() {
  ...
  const [todaysYear, setTodaysYear] = useState<number | null>(null);

  useEffect(() => {
    const loadArchive = async () => {
      setIsLoading(true);
      
      // Get today's year directly from the generator
      const currentYear = getTodaysYear();
      setTodaysYear(currentYear);
      ...
      const archiveWithToday = archiveData.map(entry => {
        ...
        if (isCurrentDay) {
          return {
            ...entry,
            year: currentYear, // Use the current year from the generator
            isToday: true
          };
        }
              ...
              {archive.slice(0, visibleDays).map((entry, index) => (
                ...
                >
                  <div className="flex justify-between items-center">
                    ...
                    <Link 
                      href={`/play?date=${entry.date}&year=${entry.year}`}
